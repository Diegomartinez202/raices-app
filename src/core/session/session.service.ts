...
export async function authenticateWithBiometric(): Promise<boolean> {
  try {
    // En este plugin el método es verifyIdentity
    await NativeBiometric.verifyIdentity({
      ...
      // Nota: Este plugin no usa 'negativeButtonText' aquí, 
      // usa los diálogos nativos del sistema.
    });
    
    return true; // Si llega aquí, es que fue exitoso
  } catch (error) {
    logger.warn('Biometría cancelada o falló:', error);
    return false;
  }
}
