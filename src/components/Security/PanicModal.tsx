...
// 1. Importamos la lógica de destrucción real
...
export const PanicModal: React.FC<PanicModalProps> = ({ onClose }) => {
  ...
  const executeSecureWipe = async () => {
    try {
      console.warn('[RAÍCES] Iniciando Protocolo de Pánico - Destrucción Física...');
      
      // 2. CONEXIÓN CRÍTICA: Llamamos al servicio de borrado seguro
      // Este método sobreescribe los archivos con patrones de bits antes de borrarlos
      await activatePanicMode(); 
      
      setStatus('done');
      // Nota: activatePanicMode() ya maneja el redireccionamiento o cierre al final
    } catch (error) {
      console.error('[RAÍCES ERROR PANIC]', error);
      alert('Error crítico en el borrado. Cierre la aplicación de inmediato.');
      ...
      <div className="bg-[#FAF8F5] p-8 rounded-2xl shadow-2xl border-2 border-[#C65D3B]/20 w-full max-w-md text-center">
        ...
        {status === 'wiping' && (
          <>
            ...
            <p className="text-[#C65D3B] text-sm font-medium italic">
              Sobreescritura física según estándar DoD 5220.22-M...
            </p>
          </>
        )}
        ...
        {status === 'done' && (
          <>
            ...
            <p className="text-[#6B5E4F] mb-6 text-sm">
              La información ha sido destruida físicamente.
            </p>
          </>
        )}
      </div>
